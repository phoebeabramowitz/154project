...
# Make sure working directory is project dircector which contains /data with all files
...
In our main data table, we remove 3686 rows where the temperature, humidity, reflective PAR, and incident PAR are all missing. 
<<>>=
