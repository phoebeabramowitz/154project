...
It is clear from these two histograms that voltage readings are not consistent between \textbf{sonoma-data-log.csv} and \textbf{sonoma-data-net.csv}.
<<eval=TRUE, echo=FALSE, warning=FALSE>>=
...
To reconcile these inconsistencies, we applied two separate conversions as to match the ranges from the paper:
...
We also noticed the ranges for hamatop and hamabot differ from the paper. They are missing a Lux to PPFD conversion in order to be consistent, which requires a 0.0185 conversion factor.
...
\section
Voltage was not consistant between data_log and data_net. For the data_net values, we scaled using....
We convert the measurements of both types of PAR from Lux to PPMG, the units expressed in the visualizations in the study.
