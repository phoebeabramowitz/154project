...
```{r}
data_net %>%
  mutate("converted_voltage" = data_net$voltage*5 / 1023) %>%
...
some result times in data net have no nodeid or epoch ..?

```{r}
length(unique(data_net$parent))
length(unique(data_net$nodeid))
length(unique(data_log$parent))
length(unique(data_log$nodeid))
length(unique(data_all$parent))
length(unique(data_all$nodeid))
```
