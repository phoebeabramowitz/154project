...
net_epoch_test <- data_net["epoch"==epoch_i]
if (nrow(net_epoch_test)>0){
  print("nice")
  #from error, return function is wrong
  return(net_epoch_test$result_time[1])
}


  for(i in nrow(data_log)){
  epoch_i <- data_log[i,"epoch"]
  net_epoch_i <- data_net["epoch"==epoch_i]
  if (nrow(net_epoch_i)>0){
    data_log[i,"result_time"] <- net_epoch_i$result_time[1]
  }
}
